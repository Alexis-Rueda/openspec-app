...
#### Scenario: Primary C button is accent styled
- **WHEN** el usuario ve los CTAs del hero
- **THEN** el botón "Ver el flujo" tiene estilo de acento (fondo verde, texto blanco)
- **AND** el botón "Comandos utili tiene estilo ghost (fondo transparente, borde)
...
- **WHEN** el Hero Section se renderiza
...
#### Scenario: Styles are scopped to component
- **WHEN** el componente está montado
- **AND** los tokens CSS globales (.os-) están disponibles como variables
- **THEN** el component no usa clases globales externas a su scope
